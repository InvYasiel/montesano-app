//---------------------------------LLAMADA CONVITES.PHP---------------------------------  
var registroConvites = [];

function recogerDatosConvites() {
    ...
    xmlhttp.onreadystatechange = function () {
        if (this.readyState == 4 && this.status == 200) {
            ...
            for (let i = 0; i < fechas.length; i++) {
                registroConvites.push(fechas[i]);
            }
            currDateConvites()
        }
    };
    xmlhttp.open("GET", "php/convites.php", true);
    console.log(xmlhttp)
    xmlhttp.send();
}

function abrirmodalConvites(dia) {
    ...
    //---------------------------------DIBURJAR GRAFICO --------------------------------- 
    for (let i = 0; i < registroConvites.length; i++) {
        if (registroConvites[i].entrada.substring(10, -1) == seleccionado) {

            for (let t = 0; t < hh.length; t++) {
                if (hh[t].id == registroConvites[i].entrada.substring(11, 16)) {
                ...
                if (hh[t].id > registroConvites[i].entrada.substring(11, 16) && hh[t].id < registroConvites[i].salida.substring(11, 16)) {
                    hh[t].style = 'background-color:rgba(165, 140, 21, 0.68);margin: 0; height:8px;width: 200px;position:relative;float:right;border-bottom:solid 1px;'
                }
                if (hh[t].id < registroConvites[i].entrada.substring(11, 16) && hh[t].id > registroConvites[i].entrada.substring(11, 16)) {
                    hh[t].style = 'background-color:rgba(165, 140, 21, 0.68);margin: 0; height:8px;width: 200px;position:relative;float:right;border-bottom:solid 1px;'
                }
                if (hh[t].id < registroConvites[i].entrada.substring(11, 16) && hh[t].id > registroConvites[i].salida.substring(11, 16)) {
                    hh[t].style = 'background-color:rgba(165, 140, 21, 0.68);margin: 0; height:8px;width: 200px;position:relative;float:right;border-bottom:solid 1px;'
    ...
    for (let i = 0; i < registroConvites.length; i++) {
        ...
        if (new Date(fechaEntrada) >= new Date(registroConvites[i].entrada) && new Date(fechaEntrada) <= new Date(registroConvites[i].salida)) {
            alert('Error esa hora está ocupada');
            v = false;
            break;
        }
        if (new Date(fechaEntrada) <= new Date(registroConvites[i].entrada) && new Date(fechaSalida) >= new Date(registroConvites[i].entrada)) {
            alert('Error esa hora está ocupada');
            v = false;
            break;
        }
        if (new Date(fechaEntrada) <= new Date(registroConvites[i].entrada) && new Date(fechaSalida) >= new Date(registroConvites[i].salida)) {
            alert('Error esa hora está ocupada');
            v = false;
            break;
        }
